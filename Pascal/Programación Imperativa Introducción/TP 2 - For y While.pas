...
begin
multiplica := num * i;
writeln(num,' x ',i,' = ',multiplica:0:2);
end;
read;
end.
// 3) escribe un nume hasta que ingresen 0 y finaliza cantidad de num ingresados
program cantnum;
var
num : real;
contador : integer;
...
while(legajo<>corte) do
    ...
    writeln('Ingresa otro legajo');
    readln(legajo);
    end;
writeln('El total de legajos es: ',contalumnos);
writeln('El total de alumnos mayor que ',promsup,' es ',cantmayor);
end.
...
program ventas;
var
ingreso,cantv,totalacum,i : integer;
begin
totalacum:=0;
for i:=1 to 2 do
begin
...
cantv:=0;
    ...
    begin
    cantv:=cantv + 1;
    totalacum:=totalacum+ingreso;    
    writeln('Ingresa montos de ventas');
    readln(ingreso)
    end;
writeln('Ventas totales dia ',i,' es ',cantv)
end;
